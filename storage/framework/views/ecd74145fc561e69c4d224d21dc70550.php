...
$__newAttributes = [];
$__propNames = \Illuminate\View\ComponentAttributeBag::extractPropNames((['carouselArticles' => [], 'latestArticles' => [], 'popularArticles' => []]));
...
foreach (array_filter((['carouselArticles' => [], 'latestArticles' => [], 'popularArticles' => []]), 'is_string', ARRAY_FILTER_USE_KEY) as $__key => $__value) {
    $$__key = $$__key ?? $__value;
}
...
<div class="w-full max-w-screen-xl mx-auto px-4 py-5" id="<?php echo e($blockId); ?>">
  ...
  <div class="grid grid-cols-1 lg:grid-cols-[1fr_2.2fr_1fr] gap-4 lg:gap-6 items-start">
    ...
    <section class="order-1 lg:order-2 flex flex-col">
      <div class="relative w-full h-[350px] sm:h-[400px] lg:h-[420px] bg-white border border-gray-200 rounded-xl overflow-hidden" id="<?php echo e($blockId); ?>-carousel">
        <div class="relative w-full h-full flex items-center justify-center pt-4 pb-10">
          
          <div class="absolute left-[-2%] sm:left-[2%] lg:left-[4%] opacity-50 blur-[2px] scale-90 cursor-pointer transition-all duration-500 rounded-2xl overflow-hidden z-10 shadow-md photo-carousel-prev-preview photo-prev" style="width: 25%; max-width: 200px; aspect-ratio: 1 / 1; background-size: cover; background-position: center; background-image: url('<?php echo e($carousel[count($carousel)-1]['image_url'] ?? asset('images/a.jpg')); ?>');"></div>

          <a href="<?php echo e(($carousel[0]['slug'] ?? '#') === '#' ? '#' : route('article.show', $carousel[0]['slug'])); ?>" class="relative z-20 shadow-2xl rounded-2xl overflow-hidden bg-white block mx-auto transition-transform duration-500 hover:scale-[1.02] photo-carousel-link photo-main-link" style="width: 52%; max-width: 350px; aspect-ratio: 1 / 1;">
            <img src="<?php echo e($carousel[0]['image_url']); ?>" alt="<?php echo e($carousel[0]['headline']); ?>" class="w-full h-full object-cover object-center bg-[#f3f4f6] photo-carousel-main-image photo-main-img" loading="lazy" />
            <div class="absolute inset-0 bg-gradient-to-t from-black/90 via-black/15 to-transparent"></div>
            <div class="absolute bottom-0 left-0 right-0 p-4">
              <h3 class="font-serif font-extrabold text-white text-[16px] leading-tight line-clamp-2 drop-shadow photo-main-title"><?php echo e($carousel[0]['headline']); ?></h3>
              <div class="text-[11px] text-white/80 mt-1.5 photo-main-time"><span><?php echo e($carousel[0]['timestamp']); ?></span></div>
            </div>
          </a>

          <div class="absolute right-[-2%] sm:right-[2%] lg:right-[4%] opacity-50 blur-[2px] scale-90 cursor-pointer transition-all duration-500 rounded-2xl overflow-hidden z-10 shadow-md photo-carousel-next-preview photo-next" style="width: 25%; max-width: 200px; aspect-ratio: 1 / 1; background-size: cover; background-position: center; background-image: url('<?php echo e($carousel[1]['image_url'] ?? asset('images/b.jpg')); ?>');"></div>
        ...
        <div class="absolute bottom-5 left-0 right-0 flex justify-center gap-2 z-50 photo-carousel-indicators photo-dots">
          <?php $__currentLoopData = $carousel; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <button type="button" class="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full transition-all duration-300 <?php echo e($i === 0 ? 'bg-black' : 'bg-gray-300 hover:bg-gray-400'); ?>" data-index="<?php echo e($i); ?>"></button>
    ...
    <aside class="order-2 lg:order-3 flex flex-col border border-gray-200 rounded-sm bg-white overflow-hidden lg:h-[420px]">
      ...
      <div class="h-[320px] lg:h-[376px] overflow-y-auto custom-scrollbar px-2 py-1 photo-tab-latest">
        <?php $__currentLoopData = $latest; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $idx => $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <a href="<?php echo e(($item['slug'] ?? '#') === '#' ? '#' : route('article.show', $item['slug'])); ?>" class="group flex items-start gap-4 py-3.5 px-3 border-b border-gray-100 last:border-b-0 hover:bg-gray-50 transition-colors">
            <span class="font-serif font-extrabold text-[32px] text-[#fca5a5] group-hover:text-[#e2231a] transition-colors shrink-0 w-8 text-center leading-none mt-1"><?php echo e(['১','২','৩','৪','৫','৬','৭','৮','৯','১০'][$idx] ?? $idx + 1); ?></span>
            <div class="flex-1 min-w-0">
              <h3 class="font-serif font-bold text-[14.5px] text-gray-800 leading-snug group-hover:text-[#e2231a] transition-colors line-clamp-2"><?php echo e($item['headline']); ?></h3>
              <div class="text-[11px] text-gray-500 mt-1.5 flex items-center gap-1">
                 ...
                 <span class="truncate"><?php echo e($item['timestamp']); ?></span>
              </div>
            </div>
          </a>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
      ...
      <div class="h-[320px] lg:h-[376px] overflow-y-auto custom-scrollbar px-2 py-1 hidden photo-tab-popular">
        <?php $__currentLoopData = $popular; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $idx => $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <a href="<?php echo e(($item['slug'] ?? '#') === '#' ? '#' : route('article.show', $item['slug'])); ?>" class="group flex items-start gap-4 py-3.5 px-3 border-b border-gray-100 last:border-b-0 hover:bg-gray-50 transition-colors">
            <span class="font-serif font-extrabold text-[32px] text-[#fca5a5] group-hover:text-[#e2231a] transition-colors shrink-0 w-8 text-center leading-none mt-1"><?php echo e(['১','২','৩','৪','৫','৬','৭','৮','৯','১০'][$idx] ?? $idx + 1); ?></span>
            <div class="flex-1 min-w-0">
              <h3 class="font-serif font-bold text-[14.5px] text-gray-800 leading-snug group-hover:text-[#e2231a] transition-colors line-clamp-2"><?php echo e($item['headline']); ?></h3>
  ...
  <script type="application/json" id="<?php echo e($blockId); ?>-data"><?php echo json_encode($carousel, 15, 512) ?></script>
</div>

<?php $__env->startPush('scripts'); ?>
<script>
document.addEventListener('DOMContentLoaded', function () {
  const block = document.getElementById('<?php echo e($blockId); ?>');
  if (!block) return;

  const dataEl = document.getElementById('<?php echo e($blockId); ?>-data');
  ...
  function articleUrl(slug) {
    if (!slug || slug === '#') return '#';
    return '<?php echo e(url('/article')); ?>/' + slug;
  }
...
<?php $__env->stopPush(); ?>
<?php /**PATH D:\Antigravity\Dhaka-Magazine-Laravel-App\resources\views/components/photo-news-block.blade.php ENDPATH**/ ?>